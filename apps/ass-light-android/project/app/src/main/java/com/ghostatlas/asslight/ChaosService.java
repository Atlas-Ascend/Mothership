...
public class ChaosService extends Service {
    ...
    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        profileIndex = clampProfile(intent == null ? 1 : intent.getIntExtra(EXTRA_PROFILE, 1));
        flashWithEvent = intent != null && intent.getBooleanExtra(EXTRA_FLASH, false);
        ...
        if (!wakeLock.isHeld()) wakeLock.acquire(MAX_SESSION_MS + 60_000L);
    ...
    private void playEvent(boolean stopAfter) {
        ...
        if (flashWithEvent) torch.pulse(180L);
    }
